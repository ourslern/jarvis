...
def gpu_status():
    try:
        ...
        return {
            "available": True, "name": name, "util_percent": int(float(util)),
            "memory_used_mb": int(float(mem_used)), "memory_total_mb": int(float(mem_total)),
            "temperature_c": int(float(temp)), "power_watts": float(power),
        }
...
ACTIONS = [
    Action("system.status", "Get CPU, RAM, disk, GPU usage, GPU temp, and GPU VRAM.", PermissionLevel.READ, status, {})
]
